...
public class GlobalFailureHandler {
    ...
    public static void handle(RoutingContext rc) {
        ...
        JsonObject error = new JsonObject().put("status", statusCode).put("error", message)
                .put("path", rc.request().path());

        if (!rc.response().ended()) {
            rc.response().setStatusCode(statusCode).putHeader("Content-Type", "application/json").end(error.encode());
        }
    }
}
