...
public class BookingHandler {
    ...
    public void bookTicket(RoutingContext rc) {
        ...
        if (body == null || !body.containsKey("flightId") || !body.containsKey("passengerId")) {
            rc.fail(400, new Exception("Missing flightId or passengerId."));
            return;
        }
        ...
        dbService.getPool().withTransaction(conn ->
                // 1. Get available seats and price (required for total_amount)
                conn.preparedQuery("SELECT available_seats, price FROM flights WHERE id = ?")
                        .execute(Tuple.of(flightId)).compose(rows -> {
                            ...
                            // 2. Update seat count and pass price to next step
                            return conn.preparedQuery("UPDATE flights SET available_seats = available_seats - 1 WHERE id = ?")
                                    .execute(Tuple.of(flightId)).map(v -> price);
                        }).compose(price -> {
                            // 3. Insert Booking with total_amount
                            String sql =
                                    "INSERT INTO bookings (booking_reference, passenger_id, flight_id, seat_number, status, total_amount) " +
                                            "VALUES (?, ?, ?, ?, 'CONFIRMED', ?)";
                            return conn.preparedQuery(sql)
                                    .execute(Tuple.of(bookingRef, passengerId, flightId, seatNumber, price));
                        }).compose(result -> {
                            // 4. Retrieve ID natively (Fixes "IDENTITY function not found")
                            Long generatedId = result.property(JDBCPool.GENERATED_KEYS).getLong(0);
                            return conn.preparedQuery("SELECT * FROM bookings WHERE id = ?").execute(Tuple.of(generatedId));
                        }).map(rows -> rows.iterator().hasNext() ? Booking.fromRow(rows.iterator().next()) : null)).onSuccess(
                booking -> rc.response().setStatusCode(201).putHeader("Content-Type", "application/json")
                        .end(Json.encodePrettily(booking))).onFailure(err -> {
            err.printStackTrace();
